package latte.Absyn; // Java Package generated by the BNF Converter.

public class BlockCons extends Block {
  public final ListStmt liststmt_;
  public BlockCons(ListStmt p1) { liststmt_ = p1; }

  public <R,A> R accept(latte.Absyn.Block.Visitor<R,A> v, A arg) { return v.visit(this, arg); }

  public boolean equals(Object o) {
    if (this == o) return true;
    if (o instanceof latte.Absyn.BlockCons) {
      latte.Absyn.BlockCons x = (latte.Absyn.BlockCons)o;
      return this.liststmt_.equals(x.liststmt_);
    }
    return false;
  }

  public int hashCode() {
    return this.liststmt_.hashCode();
  }


}
