...
Program :: { Program }
Program : ListTopDef { AbsLatte.ProgramCons $1 }
TopDef :: { TopDef }
TopDef : Type Ident '(' ListArg ')' Block { AbsLatte.FnDef $1 $2 $4 $6 }
ListTopDef :: { [TopDef] }
ListTopDef : TopDef { (:[]) $1 } | TopDef ListTopDef { (:) $1 $2 }
Arg :: { Arg }
Arg : Type Ident { AbsLatte.ArgCons $1 $2 }
ListArg :: { [Arg] }
ListArg : {- empty -} { [] }
        | Arg { (:[]) $1 }
        | Arg ',' ListArg { (:) $1 $3 }
Block :: { Block }
Block : '{' ListStmt '}' { AbsLatte.BlockCons (reverse $2) }
ListStmt :: { [Stmt] }
...
Stmt : ';' { AbsLatte.Empty }
     ...
     | Ident '[' Expr ']' '=' Expr ';' { AbsLatte.AssArr $1 $3 $6 }
     ...
     | 'for' '(' Stmt Expr ';' Stmt ')' Stmt { AbsLatte.For $3 $4 $6 $8 }
...
Expr6 : Ident { AbsLatte.EVar $1 }
      ...
      | '(' Expr ')' { $2 }
Expr5 :: { Expr }
Expr5 : 'new' Type '[' Integer ']' { AbsLatte.EArrayCons $2 $4 }
